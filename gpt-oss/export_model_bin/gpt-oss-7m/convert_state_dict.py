...
def reshape_mlp(state_dict, block_id): #model.layers.0.mlp.experts.gate_up_proj
	...
	new_state_dict = OrderedDict(state_dict)
	new_state_dict[f"block.{block_id}.mlp.mlp1_weight"] = mlp1_weight.transpose(1, 2).contiguous()
	new_state_dict[f"block.{block_id}.mlp.mlp2_weight"] = mlp2_weight.transpose(1, 2).contiguous()
