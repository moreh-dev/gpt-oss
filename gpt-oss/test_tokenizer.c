// test_tokenizer.c — Minimal encode/roundtrip tester for tokenizer.bin
//
// Build:
...
//
// Behavior:
// - Loads tokenizer.bin via gpt-oss.c's read_tokenizer().
...
int main(int argc, char **argv) {
  ...
  if (opt.roundtrip) {
    ...
    for (int i = 0; i < n_ids; i++) {
      ...
      char *piece = decode_piece(
          &tok, prev, ids[i]); // owned by tokenizer; printing is safe
      safe_printf(piece);      // safe_printf comes from gpt-oss.c
    }
    printf("\n");
  }
