// run_exec.cpp
// HIP (AMD MI250) inference runner for gpt-oss (20B-first).
// No external GPU libs; all kernels written from scratch (HIP C++).
// Keeps original CPU path untouched by building a separate binary: run_hip.
//
// Compile:
//   hipcc -O3 -std=c++17 -fopenmp -D__HIP_PLATFORM_AMD__ \
//     run_exec.cpp tokenizer.cpp -o run_hip
//
// Usage:
//   ./run_hip model.bin -m generate -i "<prompt>" -n 256
//   ./run_hip model.bin -m getp -i input.txt -o output.txt
//
// Notes for MI250 (CDNA2):
// - 64-lane wavefronts; choose block sizes as multiples of 64.
// - HBM bandwidth is king for GEMV-heavy inference. We tile reads from W and x.
// - Single-GPU, single-GCD path here (hipSetDevice(0)). Multi-GPU is a next step.
// - All math in FP32 to match CPU correctness first (optimize later).
...
static float* forward_gpu(TransformerGPU *T, int token, int pos) {
...
static void generate(TransformerGPU *T, Tokenizer *tok, Sampler *S,
                     const char *prompt, int steps) {
  if (!prompt) prompt = "";
  int *ptoks = (int*)malloc((strlen(prompt)+3)*sizeof(int));
  int ntok = 0;
  encode(tok, prompt, -1, -1, ptoks, &ntok, T->cfg.initial_context_length);
  if (ntok < 1) { fprintf(stderr,"prompt produced no tokens\n"); exit(1); }
  ...
  // print first piece
  const char *first = decode_piece(tok, 200006, token);
  safe_printf(first); fflush(stdout);

  while (pos < steps) {
    float *logits_d = forward_gpu(T, token, pos);

    // pull logits or sample on device? We sample on host for correctness parity.
    float *hlog = (float*)malloc(T->cfg.vocab_size*sizeof(float));
    HIP_CHECK(hipMemcpy(hlog, logits_d, T->cfg.vocab_size*sizeof(float), hipMemcpyDeviceToHost));

    int next;
    if (pos < ntok-1) next = ptoks[pos+1];
    else {
      next = sample_token(S, hlog);
    }
    free(hlog);
    pos++;

    if (next == 1) break; // BOS as stop per original code

    const char *piece = decode_piece(tok, token, next);
...
int main(int argc, char **argv) {
  if (argc < 3) {
    fprintf(stderr, "Usage: %s model.bin -m {generate|getp} [-i in] [-o out] [-n steps]\n", argv[0]);
    return 1;
  }
  ...
  for (int i=2;i<argc;i++) {
    if (!strcmp(argv[i], "-m") && i+1<argc) { mode=argv[++i]; }
      ...
      // if file exists, treat as file; else treat as literal string
      const char *val = argv[++i];
      if (access(val, R_OK)==0) in_file = val;
      else in_str = val;
    }
    else if (!strcmp(argv[i], "-o") && i+1<argc) out_file=argv[++i];
    else if (!strcmp(argv[i], "-n") && i+1<argc) steps=atoi(argv[++i]);
  }
  if (!mode) { fprintf(stderr,"-m required\n"); return 1; }

  TransformerGPU T;
  build_transformer(&T, ckpt);

  // tokenizer path colocated with model (same name but .bin -> .tokenizer)
  // For your base code this is handled elsewhere; here we expect tokenizer.bin
  // next to model or as tokenizer.bin in CWD; try both.
  char tok_path[PATH_MAX]; tok_path[0]='\0';
  {
    // try "./tokenizer.bin"
    if (access("tokenizer.bin", R_OK)==0) strcpy(tok_path, "tokenizer.bin");
    else {
      // try alongside model.bin replacing filename
      const char *slash = strrchr(ckpt, '/');
      if (slash) {
        size_t dirlen = (size_t)(slash - ckpt) + 1;
        strncpy(tok_path, ckpt, dirlen);
        tok_path[dirlen] = '\0';
        strcat(tok_path, "tokenizer.bin");
      } else {
        strcpy(tok_path, "tokenizer.bin");
      }
    }
  }
  Tokenizer tok;
  read_tokenizer(&tok, tok_path, T.cfg.vocab_size);

  Sampler S; build_sampler(&S, T.cfg.vocab_size, 0.0f, 0.0f, 1337ull);

  if (!strcmp(mode, "generate")) {
    const char *prompt = in_str ? in_str :
                          (in_file ? slurp_file(in_file) : "");
    generate(&T, &tok, &S, prompt, steps);
    if (in_file) free((void*)prompt);
  } else if (!strcmp(mode, "getp")) {
    if (!in_file) { fprintf(stderr,"getp requires -i <file>\n"); return 1; }
    char *prompt = slurp_file(in_file);
    // simple: generate and send to stdout or file
    if (out_file) {
      // capture stdout by redirecting temporarily
      FILE *out = fopen(out_file, "wb");
      if (!out) { fprintf(stderr,"open %s failed\n", out_file); return 1; }
      // crude: temporarily dup stdout
      int stdout_fd = dup(1);
      fflush(stdout);
      dup2(fileno(out), 1);
      generate(&T, &tok, &S, prompt, steps);
